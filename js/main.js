...
if (document.querySelector('#range-input') !== null) {
    ...
    input.oninput = (e) => {
        ...
        if (value < 5)
            img.style.transform = `scale(0.${value})`;
        else if (value === 5)
            img.style.transform = `scale(1)`;
        else
            img.style.transform = `scale(1.${value})`;

        function moveTo(e) {
            img.style.left = e.pageX / 9 - img.offsetWidth / 2 + 'px';
            img.style.top = e.pageY / 6 - img.offsetHeight / 2 + 'px';
        }

        img.style.position = 'absolute';
        document.querySelector('.account-photo-change-bl').appendChild(img);
...
if (document.querySelectorAll('.leftnav-btn') !== 0) {
    document.querySelectorAll('.leftnav-btn').forEach(function (item) {
        item.onclick = (e) => {
            e.target.closest('.leftnav-item').classList.toggle('active-left__btn');
        };
